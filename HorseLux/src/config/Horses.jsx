import horse1 from "../Images/horse-head.png";
import horse2 from "../Images/heart.png";

import horse3 from "../Images/services.png";
import horse4 from "../Images/renewable@2x.png";
import horse5 from "../Images/edit.png";
import horse6 from "../Images/thermometer.png";
import horse7 from "../Images/Beehive.png";
import horse8 from "../Images/Contacts.png";
import horse9 from "../Images/User Groups.png";
import horse10 from "../Images/Clock.png";
import horse11 from "../Images/Appointment Time.png";
import horse12 from "../Images/To Do.png";
import horse13 from "../Images/Receive Dollar.png";
import horse14 from "../Images/Contacts11.png";

import horse15 from "../Images/Upload.png";
import dental from "../Images/dental.png";
import glass from "../Images/glass.png";
import deworming from "../Images/deworming.png";
import injury from "../Images/injury.png";
import hand from "../Images/hand.png";
import veccination from "../Images/veccination.png";
import vitals from "../Images/vitals.png";
import joint from "../Images/joint.png";
import diognostics from "../Images/diognostics.png";

import ferrier from "../Images/hand.png";
import healthimg from "../Images/healthimg.png";
import medical from "../Images/diognostics.png";
import { BsChevronRight } from "react-icons/bs";






export const horse = [
  {
    id: 1,
    img: horse1,
    text: "Horse",
  },
  {
    id: 2,
    img: horse2,
    text: "Health",
  },
  {
    id: 3,
    img: horse3,
    text: "Services",
  },
  {
    id: 4,
    img: horse4,
    text: "Renewal",
  },
  {
    id: 5,
    img: horse5,
    text: "Notes",
  },
  {
    id: 6,
    img: horse6,
    text: "Temprature",
  },
  {
    id: 6,
    img: horse7,
    text: "Breeding",
  },
];

export const contact = [
  {
    id: 1,
    img: horse14,
    text: "Contact",
  },
  {
    id: 2,
    img: horse15,
    text: "Upload",
  },
  {
    id: 3,
    img: horse9,
    text: "Owner Group",
  },
];
export const schedule = [
  {
    id: 1,
    img: horse10,
    text: "Services",
  },
  {
    id: 2,
    img: horse11,
    text: "Appointment",
  },
  {
    id: 3,
    img: horse12,
    text: "Task",
  },
];

export const finance = [
  {
    id: 4,
    img: horse13,
    text: "Payment Recieved",
  },
];

export const popOver = [
  { text: "Akhal-Teke" },
  { text: "Alberta Wild Horse" },
  { text: "American Bashkir Curly" },
  { text: "American Belgian Draft" },
  { text: "American Cream Draft Horse" },
  { text: "American Drum Horse" },
  { text: "American Spotted" },
  { text: "American Walking Pony" },
  { text: "Anglo Arabian" },
  { text: "Arabian-Berber" },
  { text: "Arabian Halfbred" },
  { text: "Arabian Partbred" },
  { text: "Arabian Friesian" },
  { text: "Arasian" },
  { text: "Australian Stock Horse" },
  { text: "Australian Pony" },
  { text: "Baden-Württemberg" },
  { text: "Banker Horse" },
  { text: "Baroque Pinto" },
  { text: "Belgian Cold Blood" },
  { text: "" },
  { text: "Belgian Warmblood" },
  { text: "Brandenburg Warmblood" },
  { text: "Bosnian Mountain Horse" },
  { text: "Blazer Horse" },
  { text: "Budjonny Horses" },
  { text: "Camarillo White" },
  { text: "Campolina" },
  { text: "Canadian Horse" },
  { text: "Canadian Rustic Pony" },
  { text: "Canadian Pacer" },
  { text: "Canadian Pinto" },
  { text: "Canadian Sport Horse" },
  { text: "Canadian Warmblood" },
  { text: "Carolina Marsh Tacky" },
  { text: "Classic Pony" },
  { text: "Cleveland-Bucht" },
  { text: "Clumberland Island Horse" },
  { text: "Clydesdale" },
  { text: "Cob Horse" },
  { text: "Connemara Pony" },
  { text: "Comtois Horse" },
  { text: "Criollo Horse" },
  { text: "Curly Horse" },
  { text: "Dales Pony" },
  { text: "Danish Warmblood" },
  { text: "Dartmoor Horses" },
  { text: "Don horse" },
  { text: "Donkey" },
  { text: "Delibos" },
  { text: "Drum-Horse" },
  { text: "Dülmen wild horses" },
  { text: "English thoroughbred" },
  { text: "Exmoor Pony" },
  { text: "Falabella" },
  { text: "Fur Pony" },
  { text: "Florida Cracker Horse" },
  { text: "French Trotter" },
  { text: "Frederiksborg" },
  { text: "Freiberger" },
  { text: "Frisian horse" },
  { text: "Furioso" },
  { text: "German Trotter" },
  { text: "German Classic Pony" },
  { text: "German Riding Horse" },  
  { text: "German Riding Pony" },
  { text: "Gelderland" },
];

export const dewormingRecor= [
  { text: "ivercare - ivermectin" },
  { text: "generic - ivermectin" },
  { text: "panacur - fenbendazole" },
  { text: "panacur - powerpac - fenbendazole" },
  { text: "primex equine liquid wormer pyrantel pamoate " },
  { text: "generic - pyrantel pamoate" },
  { text: "pyrantel paste - pyrantel pamoat" },
 
];
export const InjuryData= [
  { text: "Abrasion" },
  { text: "Colic" },
  { text: "Dehydration" },
  { text: "Eye infection" },
  { text: "Eye injury " },
  { text: "Heat stroke" },
  { text: "Hoof" },
 
];
export const vitalsRecord= [
  { text: "Height" },
  { text: "Pulse" },
  { text: "Respiration" },
  { text: "Temprature" },
 
 
];
export const therapyData= [
  { text: "Acupressure" },
  { text: "Acupuncture" },
  { text: "Bodywork" },
  { text: "Bodywork" },
  { text: "Electro-stimulation " },
  { text: "Footpad" },
 
];
export const addDiognostic= [
  { text: "Blood Chemistry" },
  { text: "Bone Scan" },
  { text: "CBC" },
  { text: "Fecal" },
  { text: "Lyme Titer  " },
  { text: "MRI  " },

  { text: "Radiograph" },
 
];
export const vaccinationData= [
  { text: "eastern/western equine encephalomyel.." },
  { text: "equine herpesvirus (ehv)" },
  { text: "influenze (flu/rhino)" },
  { text: "potomac horse fever (phf)" },
  { text: "rabies " },
 
];
export const sex = [
  { text: "Gelding" },
  { text: "Mare" },
  { text: "Stallion" },
  { text: "Filly" },
  { text: "Colt" },
  { text: " Weanling" },
  { text: "Yearling" },
];
export const HorseColor = [
  { text: "Appaloosa" },
  { text: "Bay" },
  { text: "Bay Roan" },
  { text: "Black" },
  { text: "Blue Roam" },
  { text: " Brown" },
  { text: "Buck Skin" },
  { text: " Chestnut" },
  { text: " Cremello" },
  { text: " Dark Bay" },
  { text: " Dun" },
  { text: " Grey" },
  { text: " Grullo or Grulla" },
  { text: " Liver Chestnut" },
  { text: " Paint" },
  { text: " Palomino" },
  { text: " Perlino" },
  { text: " Pinto" },
  { text: " Pinto (Bay White)" },
  { text: " Pinto (Buckskin White)" },
  { text: " Pinto (Palomino White)" },
  { text: " Red Roan" },
  { text: " Roan" },
  { text: " Seal Brown" },
  { text: " Smoky Black" },
  { text: " Sorral" },
  { text: " Strawberry Roan" },
  { text: " White" },
  { text: " Other" },
  { text: " Canadian Pinto" },
  { text: " Canadian Sport Horse" },
  { text: " Canadian Warmblood" },
  { text: " Carolina Marsh Tacky" },
  { text: " Classic Pony" },
  { text: " Cleveland-Bucht" },
  { text: " Clydesdale" },
  { text: " Clumberland Island Horse" },
  { text: " Cob Horse" },
  { text: " Connemara Pony" },
  { text: " Comtois Horse" },
  { text: " Criollo Horse" },
];

export const  health = [
  {
    id: 0,
    img: glass,
    text: "Cogging",
    link: "/dashboard/selecthorse",
  },
  {
    id: 1,
    img: dental,
    text: "Dental",
    link: "/dashboard/selecthorse",
  },
  {
    id: 2,
    img: deworming,
    text: "Deworming",
    link: "/dashboard/deworming",
  },
  {
    id: 3,
    img: injury,
    text: "Injury",
    link: "/dashboard/injury",
  },
  {
    id: 4,
    img: hand,
    text: "Therapy",
    link: "/dashboard/therapy",
  },
  {
    id: 5,
    img: veccination,
    text: "Vaccination",
    link: "/dashboard/vaccination",
  },
  {
    id: 6,
    img: vitals,
    text: "Vitals",
    link: "/dashboard/vitals",
  },
  {
    id: 7,
    img: joint,
    text: "Joint Injection",
    link: "/dashboard/joint-injection",
  },
  {
    id: 8,
    img: diognostics,
    text: "Diagnostics",
    link: "/dashboard/diagnostics",
  },
  {
    id: 9,
    img: ferrier,
    text: "Ferrier",
    link: "/dashboard/addferrier",
  },
  {
    id: 10,
    img: healthimg,
    text: "General Health",
    link: "/dashboard/general-health",
  },
  {
    id: 11,
    img: medical,
    text: "Med/Supplement",
    link: "/dashboard/medSupplement",
  },
];

export const addservicesData=[
  {
    id:1,
    service:"BodyClip",
    price:"$100",
    arrow:<BsChevronRight/>

  },
  {
    id:2,
    service:"Coaching (Daily)",
    price:"$100",
    arrow:<BsChevronRight/>

  },
  {
    id:3,
    service:"Equipment",
    price:"",
    arrow:<BsChevronRight/>

  },
  {
    id:4,
    service:"Flat Rate",
    price:"$3000",
    arrow:<BsChevronRight/>

  },  {
    id:5,
    service:"Lesson",
    price:"$65",
    arrow:<BsChevronRight/>

  },  {
    id:6,
    service:"Lodging",
    price:"",
    arrow:<BsChevronRight/>

  },  {
    id:7,
    service:"Longe",
    price:"30$",
    arrow:<BsChevronRight/>

  },  {
    id:8,
    service:"Monthly Board",
    price:"$1200",
    arrow:<BsChevronRight/>

  },  {
    id:9,
    service:"Monthly Training",
    price:"$1000",
    arrow:<BsChevronRight/>

  },  {
    id:10,
    service:"Office Fee",
    price:"$25",
    arrow:<BsChevronRight/>

  },  {
    id:11,
    service:"Other",
    price:"",
    arrow:<BsChevronRight/>

  },  {
    id:12,
    service:"Riding",
    price:"$25",
    arrow:<BsChevronRight/>

  },  {
    id:13,
    service:"Show Ride",
    price:"",
    arrow:<BsChevronRight/>

  },  {
    id:14,
    service:"Travel Expense",
    price:"",
    arrow:<BsChevronRight/>

  },  {
    id:15,
    service:"Treadmill",
    price:"$25",
    arrow:<BsChevronRight/>

  },  {
    id:16,
    service:"Trim",
    price:"$30",
    arrow:<BsChevronRight/>

  },  {
    id:17,
    service:"Warmup Ride",
    price:"$50",
    arrow:<BsChevronRight/>

  },
]

export const contactType=[
  {
    contactType:"Brand Member"
  },
  {
    contactType:"Dentist"
  },  {
    contactType:"Forrier"
  },  {
    contactType:"Health Provider"
  },  {
    contactType:"Bill Payer"
  },  {
    contactType:"Barn Manager"
  },  {
    contactType:"Horse Owner"
  },  {
    contactType:"Lesse"
  },  {
    contactType:"Parent"
  },  {
    contactType:"Lesson student"
  },  {
    contactType:"Rider"
  },  {
    contactType:"Veterinarian"
  },
  {
    contactType:"Staff"
  }, {
    contactType:"Trainer"
  }, {
    contactType:"Vedor"
  }, {
    contactType:"Volunteer"
  },

  {
    contactType:"Other"
  },

]

export const dewormingRecord=[
  {
    contactType:"Brand Member"
  },
  {
    contactType:"Dentist"
  },  {
    contactType:"Forrier"
  },  {
    contactType:"Health Provider"
  },  {
    contactType:"Bill Payer"
  },  {
    contactType:"Barn Manager"
  },  {
    contactType:"Horse Owner"
  },  {
    contactType:"Lesse"
  },  {
    contactType:"Parent"
  },  {
    contactType:"Lesson student"
  },  {
    contactType:"Rider"
  },  {
    contactType:"Veterinarian"
  },
  {
    contactType:"Staff"
  }, {
    contactType:"Trainer"
  }, {
    contactType:"Vedor"
  }, {
    contactType:"Volunteer"
  },

  {
    contactType:"Other"
  },

]

